...
function prependPendingTransaction(
...
function addPendingTransactionToSnapshot(
  snapshot: TransactionSnapshot,
  transaction: StoredTransaction,
  now: Date
): TransactionSnapshot {
  const pendingSnapshot = prependPendingTransaction(snapshot, transaction);
  if (!pendingSnapshot.didInsert) {
...
async function applyRestoredCloudLedgerPendingTransactions(
  snapshot: TransactionSnapshot,
  userId: UserId
): Promise<TransactionSnapshot> {
  try {
    const pendingTransactions = await loadRestoredCloudLedgerPendingTransactions(userId);
    const now = new Date();
    return pendingTransactions.reduce(
      (currentSnapshot, transaction) =>
        addPendingTransactionToSnapshot(currentSnapshot, transaction, now),
      snapshot
    );
  } catch {
    return snapshot;
...
export async function loadInitialTransactions(db: AnyDb, userId: UserId): Promise<void> {
  ...
  try {
    ...
    const optimisticSnapshot = await applyRestoredCloudLedgerPendingTransactions(snapshot, userId);
