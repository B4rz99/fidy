...
const SENSITIVE_TEMPLATE_PATTERNS = [
  ...
  String.raw`\b(?:ref(?:erencia)?|autori[sz]aci[oó]n|authorization)\b\s*:?\s*#?\s*(?=[A-Z0-9-]{3,}\b)(?=[A-Z0-9-]*[A-Z])(?=[A-Z0-9-]*\d)[A-Z0-9-]{3,}\b`,
...
export const isAllowedStructuralTitleWord = (word: string): boolean =>
  STRUCTURAL_TITLE_WORDS.has(word);

export const isAllowedStructuralLowercaseWord = (word: string): boolean =>
