...
vi.mock("@/features/capture-sources/lib/dedup", () => ({
	findDuplicateTransaction: (...args: unknown[]) =>
		mockFindDuplicateTransaction(...args),
}));
...
function insertRetryEmail(
	overrides: Partial<typeof processedEmails.$inferInsert> = {},
) {
	const row = {
		id: "pe-retry-1",
...
describe("retry queue integration (real SQLite)", () => {
	...
	it("markForRetry updates retryCount and nextRetryAt without touching rawBody", async () => {
		...
		await markForRetry(db as any, "pe-retry-1", 2, futureTime);

		const [row] = await db
			.select()
			.from(processedEmails)
			.where(eq(processedEmails.id, "pe-retry-1"));
	...
	it("markPermanentlyFailed sets status=failed and clears rawBody", async () => {
		insertRetryEmail({ nextRetryAt: new Date().toISOString() });

		await markPermanentlyFailed(db as any, "pe-retry-1");
	...
	it("full flow: pending_retry email → successful retry → transaction created", async () => {
		...
		expect(mockParseEmailApi).toHaveBeenCalledWith(
			"Su compra por $50.000 fue aprobada en EXITO",
		);
	...
	it("full flow: pending_retry email → parse fails again → rescheduled with backoff", async () => {
		...
		mockParseEmailApi.mockRejectedValueOnce(
	...
	it("ISO timestamp comparison is correct in SQLite", () => {
		...
		const [cmp] = sqlite
			.prepare(
				"SELECT ? <= strftime('%Y-%m-%dT%H:%M:%fZ', 'now') as is_due",
			)
