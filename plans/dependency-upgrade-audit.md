...
- Main app: Expo SDK 55, React `19.2.0`, React Native `0.83.6`, new architecture enabled.
- Dependency policy currently reports 0 violations with a 365-day stale-upstream warning threshold.
- Stale-upstream findings are warnings; known incompatible or migration-heavy updates are time-boxed in `dependency-policy.json`.
...
- Keep stale-upstream findings warning-only unless they become actionable replacement work.
- Use `dependency-policy.json` for time-boxed deferrals of known incompatible or migration-heavy updates.
...
- `react-native`: upgraded in this PR to `0.83.6`; npm latest remains deferred under Expo SDK 55.
- `react-native-worklets`: upgraded in this PR to `0.7.4`; npm latest remains deferred under Expo SDK 55.
...
| `react-native` | `0.83.6` | Defer `0.85.2` | npm latest peers `react ^19.2.3`, `@types/react ^19.1.1`; npm latest engine `node ^20.19.4 || ^22.13.0 || ^24.3.0 || >=25`; Expo SDK 55 expects `0.83.6` | RN releases: `https://github.com/facebook/react-native/releases`; npm metadata checked | Native runtime, app startup, new architecture. Do not jump to `0.85.x` under SDK 55. | Upgraded to Expo-compatible `0.83.6`; defer latest in `dependency-policy.json`. |
| `react-native-worklets` | `0.7.4` | Defer `0.8.1` | npm latest `0.8.1` peers `react-native 0.81 - 0.85`, `@babel/core`, `@react-native/metro-config`; Reanimated `4.3.0` requires `0.8.x` | Reanimated/worklets docs: `https://docs.swmansion.com/react-native-worklets` | Animation compiler/runtime. Under SDK 55, use Expo expected `0.7.4`; do not move to `0.8.x` unless Reanimated moves too. | Upgraded to `0.7.4`; defer `0.8.x` in `dependency-policy.json`. |
| `@react-navigation/native` | `7.2.2` | Current | peers `react >=18.2.0`, `react-native *`; child packages target `^7.2.2` | React Navigation releases: `https://github.com/react-navigation/react-navigation/releases` | Navigation container/linking behavior; local usage in app layout and nav components. `CustomTabBar` now uses structural props to avoid duplicate nominal type instances. | Upgraded in this PR. |
| `@react-navigation/bottom-tabs` | `7.15.10` | Current installable target | peers `@react-navigation/native ^7.2.2`, `react-native-safe-area-context >=4`, `react-native-screens >=4` | React Navigation releases: `https://github.com/react-navigation/react-navigation/releases` | Bottom tab UI and custom tab bar. `7.15.11` was still inside Bun release-age quarantine. | Upgraded in this PR. |
| `@react-navigation/elements` | `2.9.15` | Current | peers `@react-navigation/native ^7.2.2`, safe-area, optional masked-view | React Navigation releases: `https://github.com/react-navigation/react-navigation/releases` | Header/elements styling risk. Expected code changes low. | Upgraded in this PR. |
| `@react-native-community/datetimepicker` | `9.1.0` | Current | peers `expo >=52`, `react *`, `react-native *`; release `9.0.0` breaking: removed deprecated Android `positiveButtonLabel`, `negativeButtonLabel`, `neutralButtonLabel`; `9.1.0` adds granular event listeners | GitHub releases reviewed: `https://github.com/react-native-datetimepicker/datetimepicker/releases` | Used in transaction/transfer/goal/financial-account/bill date fields. Search did not show deprecated button label props, so no code change was needed. | Upgraded in this PR; still needs targeted date picker QA. |
| `@react-native-community/netinfo` | `12.0.1` | Current | peers `react *`, `react-native >=0.59`; release `12.0.0` breaking: iOS 14+ minimum and Wi-Fi info now uses `NEHotspotNetwork` requiring Access Wi-Fi Information entitlement for SSID details | GitHub releases reviewed: `https://github.com/react-native-netinfo/react-native-netinfo/releases` | Local usage is `NetInfo.fetch()` in `shared/effect/network.ts`. No SSID use found, so entitlement should not affect current behavior. | Upgraded in this PR; verify offline/online sync behavior. |
...
| `@sentry/cli` | `3.4.0` | Current installable target | engine `node >=18` | Sentry CLI releases: `https://github.com/getsentry/sentry-cli/releases` | Build/source-map upload tooling. `3.4.1` was inside Bun release-age quarantine. | Upgraded in this PR. |
...
| `@supabase/supabase-js` | `2.104.1` | Current installable target | engine `node >=20`; no peer deps | Supabase JS releases: `https://github.com/supabase/supabase-js/releases` | Local Supabase client, auth session types, backup remote storage, edge calls. | Upgraded in this PR. |
| `@tanstack/react-query` | `5.100.1` | Current installable target | peers `react ^18 || ^19` | TanStack Query releases: `https://github.com/TanStack/query/releases` | Query client/provider/focus plus feature hooks. | Upgraded in this PR. |
| `posthog-react-native` | `4.43.5` | Current installable target | many optional peers including async-storage, navigation, expo-device/application/file-system/localization, session replay, safe-area, svg | PostHog JS releases: `https://github.com/PostHog/posthog-js/releases` | Local wrapper constructs `new PostHog(...)`. Peer list includes packages not all installed because many integrations are optional. | Upgraded in this PR. |
| `lucide-react-native` | `1.11.0` | Current installable target | peers `react ^16.5.1 || ^17 || ^18 || ^19`, `react-native *`, `react-native-svg ^12 || ^13 || ^14 || ^15` | Lucide releases: `https://github.com/lucide-icons/lucide/releases` | Central icon barrel in `shared/components/icons.ts`; typecheck validated exported icon names. | Upgraded in this PR; visual QA still recommended. |
| `react-native-gesture-handler` | `2.31.1` | Current | peers `react *`, `react-native *` | RNGH releases: `https://github.com/software-mansion/react-native-gesture-handler/releases` | Navigation gestures/touch handling. No direct import count beyond config/root usage. | Upgraded in this PR. |
...
| `react-native-safe-area-context` | `5.7.0` | Current | peers `react *`, `react-native *`; Expo Router requires `>=5.4.0`, current satisfies | Releases: `https://github.com/AppAndFlow/react-native-safe-area-context/releases` | Used in layout/navigation spacing. Low code-change risk, but visual QA required. | Upgraded in this PR. |
| `react-native-screens` | `4.24.0` | Current | peers `react *`, `react-native *`; Expo Router accepts any | Releases: `https://github.com/software-mansion/react-native-screens/releases` | Navigation lifecycle/native screens. Native QA required. | Upgraded in this PR. |
...
| `better-sqlite3` | `12.9.0` | Current | engine `node 20.x || 22.x || 23.x || 24.x || 25.x` | Releases: `https://github.com/WiseLibs/better-sqlite3/releases` | Test/tooling DB usage only; native Node addon. Verify local install and DB tests. | Upgraded in this PR. |
...
| `knip` | `6.6.3` | Current installable target | engine `node ^20.19.0 || >=22.12.0` | Knip releases: `https://github.com/webpro-nl/knip/releases` | Dead-code lint may change findings. Expected config-only/no code unless new findings. | Upgraded in this PR. |
...
These packages are policy warnings because latest stable is older than 365 days, not because a newer stable upgrade exists. They need periodic review, replacement consideration, or continued acceptance in the ledger.
