...
Triage issues in the current repo using a label-based state machine. Infer the repo from `git remote`. Use `gh` via the Bash tool for all GitHub operations.
...
The user invokes `github-triage` and then describes what they want in natural language. The skill interprets the request and takes the appropriate action.
...
If the issue needs to be fleshed out before it's ready for an agent, interview the maintainer to build a complete specification. If the discussion turns into domain language, scope, or terminology clarification, the `domain-model` skill is a good fit.
...
If moving to `ready-for-agent` without a grilling session, still post an agent brief comment. Keep it concise, but do not skip it: `ready-for-agent` always requires a durable handoff contract.
