...
7. Update repo or local `AGENTS.md` files if you discover genuinely reusable
   knowledge
...
If you discover a general, reusable pattern, add it to the `## Codebase
Patterns` section at the top of `scripts/ralph/progress.txt`.
...
1. Preflight check: confirm browser tooling is configured for the current
   OpenCode environment
2. If browser tooling is available, navigate to the relevant surface
3. Interact with the changed UI
4. Confirm it behaves as expected
5. Capture evidence when useful

If browser tooling is unavailable, note that manual verification is needed in
the progress log.
