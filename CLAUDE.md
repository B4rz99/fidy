...
**Platform**: React Native + Expo (NativeWind v4 / Tailwind CSS)
**Font**: Poppins (500 Medium, 600 SemiBold, 700 Bold, 800 ExtraBold)
**Icons**: `lucide-react-native` — all icons re-exported from `shared/components/icons.ts`
**Styling**: NativeWind utility classes with `className` props; dark mode via `dark:` prefix

### Token Definitions

- **Colors**: `apps/mobile/shared/constants/theme.ts` (programmatic) and `apps/mobile/tailwind.config.ts` (Tailwind classes)
- Light/dark mode pairs: `page`/`page-dark`, `primary`/`primary-dark`, `card`/`card-dark`, etc.
- Chart category colors: `chart-food`, `chart-transport`, `chart-entertainment`, etc.
- IMPORTANT: Never hardcode hex colors — always use Tailwind color tokens or `useThemeColor()` hook
- **Typography**: Tailwind `fontSize` scale — `nav` (10px), `caption` (12px), `label` (13px), `body` (14px), `section` (16px), `title` (18px), `logo` (22px), `balance` (32px)
- **Font families**: `font-poppins-medium`, `font-poppins-semibold`, `font-poppins-bold`, `font-poppins-extrabold`
- **Border radius**: `rounded-icon` (12px), `rounded-chart` (16px), `rounded-nav-pill` (36px)

### Component Organization

- Shared/reusable UI: `apps/mobile/shared/components/` — exported via `index.ts`
- Feature-scoped: `apps/mobile/features/{feature}/components/`
- Screens: `apps/mobile/app/` (Expo Router file-based routing)
- IMPORTANT: Always check `shared/components/` before creating new primitives — reuse `ScreenLayout`, `TransactionRow`, `FidyNumpad`, `SectionHeader`, `LoadingSpinner`, `ErrorFallback`, `ComingSoonScreen`
- Import shared components from `@/shared/components` (barrel export)
- Import icons from `@/shared/components/icons`
- Import RN primitives from `@/shared/components/rn` (not directly from `react-native`)

### Styling Approach

- Use NativeWind `className` for styling: `className="flex-1 bg-page dark:bg-page-dark"`
- Dark mode: add `dark:` variant for every color class
- Dynamic colors in `style` prop: use `useThemeColor("primary")` from `@/shared/hooks`
- Gap/spacing: use Tailwind `gap-*`, `p-*`, `m-*` classes or `style={{ gap: N }}`
- Platform branching: `Platform.OS === "ios"` for iOS-specific UI (native headers via `Stack.Screen`)

### Icon System

- All icons come from `lucide-react-native`, re-exported in `shared/components/icons.ts`
- Import pattern: `import { IconName } from "@/shared/components/icons"`
- Icons accept `size` and `color` props; get color via `useThemeColor()`
- IMPORTANT: DO NOT install new icon packages — use existing lucide icons
- If Figma MCP returns a localhost source for an icon/SVG, use it directly

### Asset Handling

- IMPORTANT: If the Figma MCP server returns a localhost source for an image or SVG, use that source directly
- IMPORTANT: DO NOT create placeholders if a localhost source is provided
- Static assets go in `apps/mobile/assets/`
...
5. Translate the output (React + Tailwind) into this project's conventions: NativeWind classes, `@/shared/components/rn` imports, Expo Router navigation, Zustand stores
6. Validate against Figma for 1:1 look and behavior before marking complete

### Required Code-to-Figma Flow

When creating designs in Figma from existing code, use the `figma-use` skill before every `use_figma` call and follow these conventions:
- Map `theme.ts` colors to Figma variables (light/dark modes as separate collections)
- Map `tailwind.config.ts` font sizes to Figma text styles
- Map border radius values to Figma corner radius variables
- Build components that mirror the `shared/components/` structure
- Use auto layout for all containers (maps to flexbox)
